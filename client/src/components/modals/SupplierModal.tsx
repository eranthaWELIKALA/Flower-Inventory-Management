...
export function SupplierModal({ supplier, onClose, onSave }: SupplierModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (supplier) {
        const res = await fetch(`${import.meta.env.VITE_API_BASE || 'http://localhost:4000/api'}/suppliers/${supplier.id}`, {
          method: 'PUT',
        ...
        const res = await fetch(`${import.meta.env.VITE_API_BASE || 'http://localhost:4000/api'}/suppliers`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify(dataToSave),
        });
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          <div>
            ...
            <input
              ...
              placeholder="e.g., Sunshine Flowers Inc."
            />
          </div>
